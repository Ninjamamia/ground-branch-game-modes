...
function terroristhunt:OnRoundStageSet(RoundStage)
	...
	elseif RoundStage == "PreRoundWait" then
		self.WaveNumber = 0
		self:SpawnOpFor(self.Settings.OpForCount.Value)
...
function terroristhunt:SpawnOpFor(OpForCount)
	...
	for CurrentPriorityGroup = 1, self.LastSpawnPriorityGroup do
		...
		AILeftToSpawn =  math.max( 0, OpForCount - #OrderedSpawns )
		...
		print("SpawnOpFor(): Picking max " .. CurrentAISpawnTarget .. " AI from priority group " .. CurrentPriorityGroup .. " with " .. AILeftToSpawn .. " AI left to spawn out of " .. OpForCount .. " total.")
	...
	ai.CreateOverDuration(4.0, OpForCount, OrderedSpawns, self.OpForTeamTag)
end
		...
		if CharacterController ~= nil then
			if actor.HasTag(CharacterController, self.OpForTeamTag) then
				...
				if self.WaveNumber < self.Settings.NumberOfWaves.Value then
					local OpForAliveRatio = ((#OpForControllers - 1) * 100) / self.Settings.OpForCount.Value
					print("WaveCondition: OpForAliveRatio=" .. OpForAliveRatio .. " %")
					if OpForAliveRatio <= self.Settings.WaveSpawnAtPercent.Value or #OpForControllers <= 2 then
						self.WaveNumber = self.WaveNumber + 1
						local WaveSize = math.floor((self.Settings.OpForCount.Value * self.Settings.WaveSizePercent.Value) / 100)
						print("Spawning reinforcement wave " .. self.WaveNumber .. " (" .. WaveSize .. " OpFor)")
						AdminTools:ShowDebug("Spawning reinforcement wave " .. self.WaveNumber .. " (" .. WaveSize .. " OpFor)")
						self:SpawnOpFor(WaveSize)
					end
				end
			else
